...
function isMathQuery(userText = '') {
    const t = String(userText || '').toLowerCase();
    return /(реши|вычисли|упрости|логарифм|lg|log|алгебр|уравнен|степен|дроб|корень|математ|sin|cos|tan|\d+\s*[\+\-\*\/\^]\s*\d+)/i.test(t);
}
...
async function sendMessage() {
    ...
    try {
        ...
        if (mdl.mode === 'fast') {
            systemPrompt += ' Режим быстрый: отвечай максимально быстро, но НЕ слишком коротко. Обычно 6-10 предложений, 1-2 коротких пункта если уместно.';
